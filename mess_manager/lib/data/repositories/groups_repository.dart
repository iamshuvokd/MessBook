...
class GroupsRepository {
  ...
  domain.Group _toDomain(Group row) => domain.Group(
        ...
        pollReminderMinutes: row.pollReminderMinutes,
        lowBalanceThresholdPaisa: row.lowBalanceThresholdPaisa,
        autoMealOffBelowThreshold: row.autoMealOffBelowThreshold,
  ...
  Future<void> updateGroup(domain.Group group) async {
    await (_db.update(_db.groups)..where((g) => g.id.equals(group.id))).write(
      GroupsCompanion(
        ...
        pollReminderMinutes: Value(group.pollReminderMinutes),
        lowBalanceThresholdPaisa: Value(group.lowBalanceThresholdPaisa),
        autoMealOffBelowThreshold: Value(group.autoMealOffBelowThreshold),
